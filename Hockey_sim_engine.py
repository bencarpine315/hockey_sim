...
def event_roll(off_line_a, off_line_b, def_line_a, def_line_b, event_roll_modifier=1):
    ...
    return (roll*1.8*event_roll_modifier) < a_gross_o_rating
...
def ot_gameplay(a_team_o, a_team_d, b_team_o, b_team_d, a_team_g, b_team_g, shift_time, overtime: bool):
    return gameplay(a_team_o, a_team_d, b_team_o, b_team_d, a_team_g, b_team_g, shift_time, overtime)
...
def new_gameplay(a_team_o, a_team_d, b_team_o, b_team_d, a_team_g, b_team_g, shift_time, overtime: bool):
    time=0
    a_score = 0
    a_shots = 0
    b_score = 0
    ...
    while time < shift_time:
        if just_scored==True and overtime==True:
            ...
            if faceoff(a_team_o, b_team_o)=='A':
                center = get_pos(a_team_o, a_team_d, "C")
                assists = [center]
                o_off, o_def, d_off, d_def, d_g = a_team_o, a_team_d, b_team_o, b_team_d, b_team_g
                continue
            else:
                center = get_pos(b_team_o, b_team_d, "C")
                assists = [center]
                o_off, o_def, d_off, d_def, d_g = b_team_o, b_team_d, a_team_o, a_team_d, a_team_g
                continue
        else:
            d_g_row = d_g.iloc[0]
            ...
            if check_event == True:
                ...
                if pass_chance[0] == True:
                    ...
                    my_event_roll_modifier, my_shot_mod = my_event_roll_modifier*(pass_chance[1]), my_shot_mod*1.05
                    continue
                ...
                if rand.randint(0,1)==0:
                    ...
                    assists = [ld]
                    # print(f'{make_list(new_assists)} is the current assists list. {ld["Name"]} {ld["Surname"]} was added after a takeaway, as he is the defenseman.')
                    o_off, o_def, d_off, d_def = d_off, d_def, o_off, o_def
                    continue
                else:
